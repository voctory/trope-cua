...
enum VersionCheck {
    ...
    private static let repo = "voctory/trope-cua"
    private static let tagPrefix = "trope-cua-v"

    /// Fetch the latest trope-cua release tag from GitHub.
    ...
    static func fetchLatest(timeout: TimeInterval = 4) async -> String? {
        ...
        req.setValue("trope-cua/\(CuaDriverCore.version)", forHTTPHeaderField: "User-Agent")
    ...
    static func warnIfOutdated() {
        Task.detached(priority: .background) {
            ...
            fputs(
                "⚠  trope-cua \(latest) is available (you have \(CuaDriverCore.version))."
                + " Run: trope-cua update\n",
                stderr
            )
        }
    }
