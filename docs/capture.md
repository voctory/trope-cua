...
`get_window_state` defaults to `capture_mode=som` and returns the UIA tree plus a JPEG screenshot capped to a 1568px longest side. Use `capture_mode=ax` for cheap tree-only refreshes when pixels are not needed. Set `max_image_dimension=0` if a workflow needs native screenshot resolution; use `zoom` for local detail.
